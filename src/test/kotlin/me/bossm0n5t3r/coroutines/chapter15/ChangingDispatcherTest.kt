...
class ChangingDispatcherTest {
    interface CsvReader {
        fun <T> readCsvBlocking(fileName: String, clazz: Class<T>): GameState
    }

    class GameState

    class SaveReader(private val reader: CsvReader) {
        suspend fun readSave(name: String): GameState =
            withContext(Dispatchers.IO) { reader.readCsvBlocking(name, GameState::class.java) }
    }
    ...
    fun `should change dispatcher`() = runBlocking {
        ...
        every { csvReader.readCsvBlocking(aFileName, GameState::class.java) } coAnswers
            {
        ...
        // when
        withContext(newSingleThreadContext(startThreadName)) { saveReader.readSave(aFileName) }
