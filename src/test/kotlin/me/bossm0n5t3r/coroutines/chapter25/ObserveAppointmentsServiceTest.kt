...
class ObserveAppointmentsServiceTest {
    class FakeAppointmentRepository(private val flow: Flow<AppointmentsEvent>) :
    ...
    fun `should keep only appointments from updates`() = runTest {
        ...
        // then
        assertEquals(listOf(listOf(anAppointment1), listOf(anAppointment2)), result)
    }

    @OptIn(ExperimentalCoroutinesApi::class)
    @Test
    fun `should eliminate elements that same as the previous one`() = runTest {
        // given
        val repo =
            FakeAppointmentRepository(
                flow {
        ...
        // when
        val result = service.observeAppointments().map { currentTime to it }.toList()
    ...
    fun `should retry when API exception`() = runTest {
        ...
        // when
        val result = service.observeAppointments().take(3).toList()

        // then
        assertEquals(
            listOf(listOf(anAppointment1), listOf(anAppointment1), listOf(anAppointment1)),
            result,
        )
    }
    ...
    fun `should retry when API exception with the code 5XX`() = runTest {
        ...
        // when
        val result = service.observeAppointments().catch<Any> { emit(it) }.toList()

        // then
        assertTrue(retried)
        assertEquals(listOf(listOf(anAppointment1), listOf(anAppointment1), someException), result)
    }
}
